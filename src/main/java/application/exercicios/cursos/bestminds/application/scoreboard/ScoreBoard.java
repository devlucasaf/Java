...
public class ScoreBoard {
    ...
    private static void criarTelaInicial() {
        ...
        // Campos de texto
        JTextField txtFieldCampoJogador1 = criarCampoTexto(painel, "Jogador 1", 95);
        JTextField txtFieldCampoJogador2 = criarCampoTexto(painel, "Jogador 2", 195);
        JTextField txtFieldCampoSets = criarCampoTexto(painel, "3", 295);
        JTextField txtFieldCampoPontos = criarCampoTexto(painel, "5", 395);
        ...
        botaoComecar.addActionListener(e -> {
            try {
                String nomeJogador1 = txtFieldCampoJogador1.getText().trim();
                String nomeJogador2 = txtFieldCampoJogador2.getText().trim();
                int sets = Integer.parseInt(txtFieldCampoSets.getText().trim());
                int pontos = Integer.parseInt(txtFieldCampoPontos.getText().trim());
    ...
    private static void criarRotulo(JPanel painel, String texto, int x, int y) {
        JLabel lblRotulo = new JLabel(texto);
        lblRotulo.setBounds(x, y, 200, 50);
        painel.add(lblRotulo);
    }

    private static JTextField criarCampoTexto(JPanel painel, String textoInicial, int y) {
        JTextField txtFieldCampo = new JTextField(textoInicial);
        txtFieldCampo.setBounds(10, y, 320, 50);
        painel.add(txtFieldCampo);
        return txtFieldCampo;
    }
