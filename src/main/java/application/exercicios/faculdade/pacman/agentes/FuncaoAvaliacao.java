...
public final class FuncaoAvaliacao {
    ...
    public static double avaliar(EstadoJogo estado) {
        if (estado.venceu()) return 1_000_000.0;
        if (estado.perdeu()) return -1_000_000.0;
        ...
        for (int x = 0; x < comidas.length; x++) {
            for (int y = 0; y < comidas[x].length; y++) {
                if (comidas[x][y]) {
                    ...
                    if (d < comidaMaisProxima) comidaMaisProxima = d;
                }
            }
        }

        if (totalComida == 0) return score + 1_000_000.0;
        ...
        for (EstadoAgente f : fantasmas) {
            ...
            if (f.estaAssustado()) {
                if (d < distFantasmaAssustado) distFantasmaAssustado = d;
            } else {
                if (d < distFantasmaAtivo) distFantasmaAtivo = d;
            }
        }

        if (distFantasmaAtivo != Double.POSITIVE_INFINITY) {
            if (distFantasmaAtivo <= 1) score -= 5000;
            else score -= 20.0 / distFantasmaAtivo;
        }
        ...
        if (!capsulas.isEmpty() && distFantasmaAtivo < 5) {
            ...
            for (Posicao c : capsulas) {
                ...
                if (d < capMaisProx) capMaisProx = d;
            }
